...
void ReadImage(Mat &image ,unsigned char pathNum, int num, bool flag)
{
	...
	{
		sprintf(filename, "E:/咸鱼/2020.04/C++/OPENCV SVM MNIST/OPENCV+SVM+MNIST/MNIST图片库/MNIST图片库/trainimage/pic2/%d/%d.bmp", pathNum, num);//用这个函数来转换图片名称，存放在filename中。
	}
	else   //反之则为测试样本路径
	{
		sprintf(filename, "E:/咸鱼/2020.04/C++/OPENCV SVM MNIST/OPENCV+SVM+MNIST/MNIST图片库/MNIST图片库/testimage/pic2/%d/%d.bmp", pathNum, num);//用这个函数来转换图片名称，存放在filename中。
	}
