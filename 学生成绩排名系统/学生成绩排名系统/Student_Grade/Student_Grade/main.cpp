...
private:
	...
	student_Node *next;
public:
	student_Node(string na="空",long num=0,string cla="空")		//对成绩和姓名所以成员都进行初始化
		:Less(L)
	{
		SetName(na);
		SetNumber(num);
	...
	{
		cout<<left<<setw(8)<<rhs.Name<<left<<setw(15)<<rhs.Number<<left<<setw(10)<<rhs.Class<<left<<setw(8)<<rhs.Less.ReturnMount_s()<<left<<setw(8)<<rhs.Less.ReturnAverg_s()<<endl;
...
public:
	...
	void Display(int x=1)				//显示输出cout<<left<<setw(40)<<"靠左输出"<<left<<setw(35)<<"靠左输出"<<endl;
	{
		if(x==1)
			Fileread();
		cout<<left<<setw(5)<<"·序号·"<<left<<setw(8)<<" ·姓名·"<<left<<setw(15)<<" ·学号·"<<left<<setw(10)<<"·班级·"<<left<<setw(8)<<"|总分|"<<left<<setw(8)<<"|平均分|"<<endl<<endl;
		student_Node *p=head->next;
		int i=1;
		while(p!=NULL)
		{
	...
	void Modify_small(student_Node *p)
	{
		...
		{
			cout<<"-------------------------------------------------------"<<endl;
			cout<<"请选择修改选项   1.姓名 2.学号 3.班级 4.成绩";
            cin>>a;
			cout<<"-------------------------------------------------------"<<endl;
			switch(a)
			{
